...
вертикальному слайсу. Текущий код — demo-лендинг, Lit-виджет, Node API,
TravelRun runtime и агентный слой для живого запроса.
...
выбирает сохранённый сценарий или живой run. `presentationStep` передаётся в
виджет только для выбора локального scenario state; provenance, событие, оценка
такси и числовая summary-карточка показываются на demo-лендинге. `TripWidgetState`
и `TravelRunSseEvent` не расширены. Поэтому открытый SSE остаётся подключённым
при переходе в сценарий, а накопленный `sessionState` виден после возврата.
...
DOM отвечает за чат, карточки, progress, действие и open/collapse-поведение.
...
- ADR-009 про разделение объясняющего лендинга и рабочего виджета;
